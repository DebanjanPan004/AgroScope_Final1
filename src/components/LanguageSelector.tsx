  ...
  return (
    <div ref={containerRef} className={cn("relative", className)}>
      ...
      >
        ...
        <span className={isNavbar ? "text-gray-500 text-xs" : "text-white/60 text-xs"}>▼</span>
      </button>

      {open && (
        <div
          className="absolute right-0 top-full mt-1 z-50 w-72 rounded-xl border border-gray-200 bg-white shadow-xl overflow-hidden"
          style={{ maxHeight: 280 }}
        >
          <div className="p-2 border-b border-gray-100">
            <input
              ...
              className="w-full rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-sm text-gray-800 placeholder:text-gray-500 focus:outline-none focus:ring-2 focus:ring-[var(--crop-green)]/50"
            />
          </div>
              ...
              <>
                ...
                {indianFiltered.map((lang) => (
                  <button
                    ...
                    className={cn(
                      "w-full flex items-center gap-2 px-3 py-2 text-left text-sm text-gray-800 hover:bg-gray-100 transition-colors",
                      selected.code === lang.code && "bg-green-50"
                    )}
                  >
                    <span>{lang.flag}</span>
                    <span className="flex-1 truncate">
                      <span className="text-gray-900">{lang.native}</span>
