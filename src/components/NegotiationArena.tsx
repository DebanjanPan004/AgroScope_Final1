          ...
          <div className="shrink-0 px-4 pt-3 pb-3 border-b border-white/10 bg-[var(--field)]">
            <button
              ...
              onMouseEnter={() => {
                try {
                  fetch(
                    `/api/forecast/ai-30days?wasteType=${encodeURIComponent(wasteTypeLabelToKey(wasteType))}&city=${encodeURIComponent(city || 'Chennai')}&quantity=${encodeURIComponent(String(quantityTons || 1))}`
                  ).catch(() => {});
                } catch (_) {}
              }}
