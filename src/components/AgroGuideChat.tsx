...
import { getAgroGuideResponse, getAgroGuideStatus, type AgroGuideMessage } from "@/services/agroGuideService";
...
const AGROGUIDE_ROUTES = [
  ...
  "/startup-input", "/farmer-inventory", "/startup-matches", "/agro-news-live", "/weather-forecast",
  "/home", "/input", "/profile", "/notifications", "/forecast", "/carbon", "/recommendations",
  "/loyalty", "/dashboard", "/results",
];
...
const WELCOME_MESSAGE: AgroGuideMessage = {
  ...
  role: "assistant",
  content:
    "Namaste! 🌾 I'm AgroGuide — your personal navigation assistant for AgroScope.\n\nI know every page, every feature, every formula, and the complete tech stack. Ask me anything about this platform.\n\nWhat would you like to know? 🌱",
  timestamp: Date.now(),
};
...
export default function AgroGuideChat({
  ...
  const handleVoiceTranscript = (text: string, isFinal: boolean) => {
    setInputText(text);
    if (isFinal && text.trim()) setTimeout(() => sendMessage(text.trim()), 800);
  };

  return (
    <motion.div
      className="fixed bottom-0 right-0 z-[9998] flex h-[85vh] w-full max-w-[100vw] flex-col overflow-hidden rounded-t-2xl border border-gray-200 shadow-2xl sm:bottom-[90px] sm:right-6 sm:h-[560px] sm:w-[380px] sm:max-w-[380px] sm:rounded-2xl"
    ...
    >
      ...
      {/* Input bar */}
      <div className="shrink-0 flex gap-2 border-t border-gray-200 bg-white p-4">
        <input
          type="text"
          value={inputText}
          ...
          className="flex-1 rounded-xl border border-gray-300 bg-gray-50 px-4 py-3 text-sm text-gray-900 placeholder:text-gray-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-400"
          disabled={isLoading}
        />
        <VoiceInputButton
