...
export function usePriceNegotiation(
  ...
  const fetchMarketPrice = useCallback(async (): Promise<MarketPrice | null> => {
    ...
    try {
      const res = await fetch(
        `/api/price-negotiation/market-price?wasteType=${encodeURIComponent(wasteType)}&city=${encodeURIComponent(city)}`
      );
  ...
  const sendMessage = useCallback(
    async (userText: string) => {
      ...
      try {
        ...
        const res = await fetch('/api/price-negotiation/chat', {
          method: 'POST',
    ...
    [wasteType, quantityTons, city, livePricePerKg, liveTrend, selectedLanguage]
  );
  ...
  return { state, fetchMarketPrice, sendMessage, reset };
}
