...
const categoryStyles: Record<string, { bg: string; text: string; border: string; glow: string; badge: string }> = {
  Policy: { bg: "bg-blue-500/10", text: "text-blue-700", border: "border-blue-500/30", glow: "hover:shadow-blue-500/20", badge: "bg-blue-500" },
  Market: { bg: "bg-green-500/10", text: "text-green-700", border: "border-green-500/30", glow: "hover:shadow-green-500/20", badge: "bg-green-500" },
  Environment: { bg: "bg-emerald-500/10", text: "text-emerald-700", border: "border-emerald-500/30", glow: "hover:shadow-emerald-500/20", badge: "bg-emerald-500" },
  Technology: { bg: "bg-amber-500/10", text: "text-amber-700", border: "border-amber-500/30", glow: "hover:shadow-amber-500/20", badge: "bg-amber-500" },
};
...
function EnhancedNewsCard({ news, index, isLive = false, isBookmarked, onBookmark, viewMode, isDarkMode }: NewsCardProps) {
  ...
  const style = categoryStyles[news.category] || categoryStyles.Market;
...
export default function AgroNewsLive() {
  ...
  const [globalNews, setGlobalNews] = useState<AgriNewsItem[]>([]);
  ...
  const detectUserLocation = async () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (error) => {
        ...
        toast({ title: "Location denied", description: "Enable GPS/location permission to get local news.", variant: "destructive" });
        setIsLoadingLocation(false);
      },
