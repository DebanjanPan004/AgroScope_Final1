...
import { CropBadge, CROP_WASTE_CONFIG } from "@/lib/cropIcons";
...
/** Final display values by crop (fallback when API fails). Must match backend FINAL_FORECAST. */
...
const Input = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Enable "Continue to Impact" after a short delay so the flow feels instant; POST still runs in background
    ...
    try {
      ...
      try {
        // AgroCredits = trees equivalent (1 tree = 1 AgroCredit). Use Carbon Impact result or compute.
    ...
    <div className="min-h-screen bg-gradient-to-b from-background to-muted/30">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <Select value={wasteType} onValueChange={setWasteType}>
                  <SelectTrigger id="waste-type" className="h-12">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    {categories.map((c) => (
                      <SelectItem key={c} value={c}>
                        {(() => {
                          const config = resolveCropKeyFromLabel(c);
                          if (!config) return c;
              ...
              {/* Optional: Price per kg + Fair Price Guarantee */}
              <div className="space-y-2">
                <SectionHeader
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Live market oracle · DeepSeek + Tavily · Cinematic negotiation
                  </p>
                </div>
                ...
                {/* Market status from direct input: < 50 Below, 50–100 Current, > 100 Above */}
              ...
              {/* Quality & Moisture (for farmer rating) */}
              ...
              <div className="space-y-2">
                ...
                {/* Nearest Cold Storage Hub – simple dropdown below location */}
                ...
                <div className="flex items-center justify-between gap-4 flex-wrap">
                  <div>
                    ...
                    <p className="font-medium flex items-center gap-2 mt-1">
                      ...
                      <a
                        href="http://localhost:5173/carbon"
                        target="_blank"
              ...
              {/* Waste-to-product recommendations */}
              <Card className="p-4 bg-muted/30">
              ...
              {/* Farmer rating (shown when we have a value, e.g. after submit) */}
              {farmerRating != null && (
              ...
              {/* Save provision then navigate to farmer inventory on success */}
              <div className="pt-4">
                <Button 
                  type="submit"
                  variant="cta" 
      ...
      {showWeightEstimator && (
        <WeightEstimatorModal
          wasteType={wasteType ? wasteTypeLabelToKey(wasteType) : undefined}
          wasteLabel={wasteType || "Paddy Husk"}
          cropType={wasteType}
          onApplyWeight={handleApplyWeightKg}
