...
import { ArrowRight, Building2, Loader2, ShieldCheck, Sparkles, Star, TrendingUp } from "lucide-react";
...
function CompanyRow({ company, rank }: { company: LoyaltyCompanyProfile; rank?: number }) {
  ...
  return (
    <div className="grid grid-cols-[1fr_auto_auto] items-center gap-3 rounded-2xl border border-black/10 bg-white px-4 py-3">
      <div className="flex min-w-0 items-center gap-3">
        {showLogo ? (
          <img
            src={company.logoUrl}
            alt={`${company.name} logo`}
            className="h-10 w-10 rounded-md border border-black/10 bg-white p-1 object-contain"
            loading="lazy"
            referrerPolicy="no-referrer"
            onError={() => setLogoErrored(true)}
          />
        ) : (
          <div className="grid h-10 w-10 shrink-0 place-items-center rounded-md border border-black/10 bg-white">
        ...
        <div className="min-w-0">
          ...
          <p className="truncate text-xs text-black/60">{company.segment || "Startup/Buyer"}</p>
        </div>
      </div>
      <p className="text-right text-sm font-black text-black">{company.score.toFixed(1)}</p>
    ...
    <div className="min-h-screen bg-[#e6e8eb] px-4 py-8 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl space-y-6">
        <motion.section
          ...
          className="relative overflow-hidden rounded-[28px] border border-black/10 bg-white p-6 md:p-8"
        >
          ...
          <div className="relative grid gap-6 lg:grid-cols-[1fr_auto]">
            <div>
              <div className="inline-flex rounded-full border border-black/10 bg-black px-4 py-1 text-[11px] uppercase tracking-[0.24em] text-white">
                Startup Loyalty Dashboard
              </div>
              <h1 className="mt-4 font-display text-4xl font-black leading-tight text-black md:text-5xl">
            ...
            <div className="min-w-[250px] rounded-2xl border border-black/10 bg-[#f7f7f8] p-4">
              <p className="text-xs uppercase tracking-[0.2em] text-black/60">Tier Distribution</p>
              <div className="mt-3 space-y-2">
                {tierStats.map((row) => (
                  <div key={row.key} className="flex items-center justify-between rounded-xl bg-white px-3 py-2 text-sm">
                    ...
                    <span className="font-bold text-black">{row.count}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <section className="grid grid-cols-1 gap-6 xl:grid-cols-12">
          ...
          >
            <Card className="rounded-[24px] border border-black/10 bg-[#f7f7f8] shadow-none">
              <CardContent className="p-6">
                <div className="mb-4 flex items-center justify-between">
                  <div>
                    <h2 className="text-2xl font-bold text-black">Top Companies</h2>
                    <p className="text-sm text-black/60">Top 5-10 ranked by weighted loyalty score.</p>
                  </div>
                ...
                {!loading && !error ? (
                  <div className="space-y-2.5">
                    ...
                    {topCompanies.length === 0 ? <p className="text-sm text-black/60">No ranked companies available yet.</p> : null}
                  </div>
                ) : null}
              </CardContent>
            </Card>
            ...
            <Card className="rounded-[24px] border border-black/10 bg-[#f7f7f8] shadow-none">
              <CardContent className="p-6">
                <div className="mb-4 flex items-center justify-between">
                  <h2 className="text-xl font-bold text-black">All Listed Companies</h2>
                  <span className="rounded-full bg-white px-3 py-1 text-xs font-semibold text-black">Total: {allCompanies.length}</span>
                </div>
          ...
          >
            <Card className="overflow-hidden rounded-[24px] border border-black/10 bg-black shadow-none">
            ...
            <Card className="rounded-[24px] border border-black/10 bg-[#f7f7f8] shadow-none">
              <CardContent className="p-6">
                <div className="mb-4 flex items-center justify-between">
                  <h3 className="text-lg font-bold text-black">Tier Metrics</h3>
                  <TrendingUp className="h-4 w-4 text-black/60" />
                </div>
                <div className="space-y-2.5">
                  {Object.entries(data?.breakdown || {}).map(([key, value]) => (
                    <div key={key} className="rounded-xl border border-black/10 bg-white p-3">
                      <div className="mb-1 flex items-center justify-between text-xs uppercase tracking-[0.12em] text-black/55">
                        <span>{key}</span>
            ...
            <Card className="rounded-[24px] border border-black/10 bg-[#f7f7f8] shadow-none">
              <CardContent className="space-y-3 p-6">
                <h3 className="text-lg font-bold text-black">Open Tier Pages</h3>
                <p className="text-sm text-black/60">Compare Tier A/B/C company matrices and reasoning.</p>
                {(["A", "B", "C"] as const).map((tier) => (
                  <Link key={tier} to={`/loyalty/tier/${tier}`} className="block">
                    <Button className="w-full justify-between rounded-xl border border-black/20 bg-white text-black hover:bg-green-500 hover:text-white">
